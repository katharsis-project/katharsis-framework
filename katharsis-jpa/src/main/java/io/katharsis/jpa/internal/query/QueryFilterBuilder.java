...
public final class QueryFilterBuilder<P, F> {
  ...
  protected P filterSpecListToPredicate(MetaDataObject rootMeta, F root, FilterSpec fs, boolean forceEntityBased,
    ...
    if ((fs.getOperator() == FilterOperator.EQ || fs.getOperator() == FilterOperator.NEQ) && fs.getValue() instanceof Collection
  ...
  private P filterExpressions(FilterSpec fs, MetaDataObject rootMeta, F root, boolean forceEntityBased,
    ...
    if (fs.getOperator() == FilterOperator.NOT) {
      return backend.not(backend
          .and(filterSpecListToPredicateArray(rootMeta, root, fs.getExpression(), forceEntityBased, defaultPredicateJoinType)));
    }
      ...
      return backend
          .or(filterSpecListToPredicateArray(rootMeta, root, fs.getExpression(), forceEntityBased, defaultPredicateJoinType));
    }
    else {
