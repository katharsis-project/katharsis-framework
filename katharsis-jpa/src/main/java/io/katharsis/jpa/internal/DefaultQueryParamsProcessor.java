...
import io.katharsis.jpa.QueryParamsProcessor;
...
public class DefaultQueryParamsProcessor implements QueryParamsProcessor {

	private ResourceRegistry resourceRegistry;

	public DefaultQueryParamsProcessor(ResourceRegistry resourceRegistry) {
		this.resourceRegistry = resourceRegistry;
	}

	@Override
	public <T> void prepareExecution(QueryExecutor<T> executor, QueryParams queryParams) {
		applyPaging(executor, queryParams);
		applyRelatedFields(executor, queryParams);
	}
	...
	protected <T> void applyFiltering(QueryBuilder<T> builder, QueryParams queryParams) {
		...
		if (filters != null && !filters.getParams().isEmpty()) {
			...
			if (filters.getParams().size() > 1 || filterParams == null)
				throw new UnsupportedOperationException("only " + resourceType + " repositoryType supported for filtering, got " + filters.getParams().keySet());
	...
	protected <T> void applySorting(QueryBuilder<T> builder, QueryParams queryParams) {
		...
		if (sorting != null && !sorting.getParams().isEmpty()) {
			...
			if (sorting.getParams().size() > 1 || sortingParams == null)
				throw new UnsupportedOperationException("only " + resourceType + " repositoryType supported for sorting, got " + sorting.getParams().keySet());
	...
	protected <T> void applyRelatedFields(QueryExecutor<T> executor, QueryParams queryParams) {
		...
		if (includedRelations != null && !includedRelations.getParams().isEmpty()) {
			...
			// no support to include relations of relations yet (specify path from root instead)
			if (includedRelations.getParams().size() > 1 || includedRelationsParams == null)
				throw new UnsupportedOperationException("only " + resourceType + " repositoryType supported to include relations, got " + includedRelations.getParams().keySet());
