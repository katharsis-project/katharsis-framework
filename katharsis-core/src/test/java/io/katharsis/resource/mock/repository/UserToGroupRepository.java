...
public class UserToGroupRepository implements BulkRelationshipRepositoryV2<User, Long, Group, Long> {
	...
	@Override
	public MultivaluedMap<Long, Group> findTargets(Iterable<Long> sourceIds, String fieldName, QuerySpec querySpec) {
		MultivaluedMap<Long, Group> map = new MultivaluedMap<>();
	...
	@Override
	public Group findOneTarget(Long sourceId, String fieldName, QuerySpec querySpec) {
	...
	public ResourceList<Group> findManyTargets(Long sourceId, String fieldName, QuerySpec querySpec) {
		ResourceList<Group> projects = new DefaultResourceList<>();
		for (Relation<User> relation : THREAD_LOCAL_REPOSITORY.keySet()) {
			if (relation.getSource().getId().equals(sourceId) && relation.getFieldName().equals(fieldName)) {
				Group project = new Group();
				project.setId((Long) relation.getTargetId());
	...
	@Override
	public Class<Group> getTargetResourceClass() {
		return Group.class;
	}
